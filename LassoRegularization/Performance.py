...
def calculate_weights(training_records, output):
        ...
        for i in range(NUM_OF_ITERATIONS):
            ...
            weights = LassoRegression.soft_thresholding_operator(weights - LEARNING_RATE*slope,
                                            LEARNING_RATE*LASSO_REGULARIZATION_STRENGTH)
...
for i in range(0,100):
    ...
    itr,mse_costs = start_lasso_regression(training_records=training_rec, output=out)
...
print "Average learning speed for L1 Regression is: ", avgSpeed


#PLOTS
x = np.arange(0,100,1)
y = learningSpeeds
plt.bar(x, y, 0.2, color='b', label='Learning Speed - L1 Regularizer')
plt.errorbar(x, y, xerr=0, yerr=0)


plt.xlabel('Iterations on Different Datasets')
plt.title('L1 Regularizer - Learning Speed\nAverage speed is {:.2f} iterations for Alpha = {:.2f}'.format(avgSpeed, LEARNING_RATE))
