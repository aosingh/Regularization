...
def calculate_weights(training_records, output):
        ...
        for i in range(NUM_OF_ITERATIONS):
            ...
            weights -= (LEARNING_RATE * (slope + (RIDGE_LEARNING_RATE/len(output))*weights))
...
for i in range(0,100):
    ...
    itr,mse_costs = start_ridge_regression(training_records=training_rec, output=out)
...
print "Average learning speed for L2 Regression is: ", avgSpeed


#PLOTS
x = np.arange(0,100,1)
y = learningSpeeds
plt.bar(x, y, 0.2, color='b', label='Learning Speed - L2 Regularizer')
plt.errorbar(x, y, xerr=0, yerr=0)


plt.xlabel('Iterations on Different Datasets')
plt.title('L2 Regularizer - Learning Speed\nAverage speed is {:.2f} iterations for Alpha = {:.2f}'.format(avgSpeed, LEARNING_RATE))
